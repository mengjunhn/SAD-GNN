...
# 这里batch_size你运行的时候改回512
parser.add_argument('--weight_decay', type=float, default=5e-4)
parser.add_argument('--neg_samples', type=int, default=1)
parser.add_argument('--data_size_ratio', type=int, default=1)
parser.add_argument('--use_cuda', type=bool, default=True, choices=[0, 1])
parser.add_argument('--pkl_name', type=str, default=root_path + '/drugbank_test/inductive_drugbank.pkl')
...
df_ddi_s1 = pd.read_csv(root_path + '/drugbank_test/inductive_data/fold3/s1.csv')
df_ddi_s2 = pd.read_csv(root_path + '/drugbank_test/inductive_data/fold3/s2.csv')
...
s1_tup = s1_tup[:int(len(s1_tup) * ratio)]
s2_tup = s2_tup[:int(len(s2_tup) * ratio)]
...
num_workers = 0
s1_data_loader = DrugDataLoader(s1_data, batch_size=batch_size * 3, num_workers=num_workers)
s2_data_loader = DrugDataLoader(s2_data, batch_size=batch_size * 3, num_workers=num_workers)
...
model = models.MVN_DDI(n_atom_feats, n_atom_hid, kge_dim, rel_total, heads_out_feat_params=[64, 64],
                       blocks_params=[2, 2])
